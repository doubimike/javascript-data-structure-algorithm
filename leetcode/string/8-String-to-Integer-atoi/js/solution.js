/*
* @Author: zhanghang
* @Date:   2018-02-09 23:53:29
* @Last Modified by:   zhanghang
* @Last Modified time: 2018-02-10 00:02:56
*/
...
// 先把字串前面可能出現的空白字串都消除，找到第一個出現得+-符號或數字。
// 如果剩下的開頭是+-符號，先用一個變數存放起來，然後把他截掉。
// 如果輸入的字串是合法的，理論上現在開頭就應該只剩下數字，如果又出現+-號或其他字元，就可以判定為不合法字串。
// 經過上面3取到最後都是數字，不過這沒影響。關，剩下的開頭肯定是數字了，這代表我們一定可以解析出一個整數出來，接下來將開頭到不是數字字元中間的數字字串取出來，當然也有可能一直到最後都是數字，不過這沒影響。
// 結合剛才儲存的正負符號與上一個步驟拿到的數字字串後轉換成整數，這時候還要判斷整數是否超過int的最大值(2147483647)與最小值(-2147483648)，超過的話上面題目已經說明過要怎麼處理，沒超過就直接回傳解析後的整數。
